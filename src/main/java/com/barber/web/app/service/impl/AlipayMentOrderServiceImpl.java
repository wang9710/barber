package com.barber.web.app.service.impl;
...
import com.barber.web.crm.service.OrderService;
import com.barber.web.utils.ApiResult;
import com.barber.web.utils.enums.HttpStateEnum;
import com.barber.web.utils.enums.IsPaidEnum;
import com.barber.web.app.co.OrderCo;
import com.barber.web.app.model.AlipayConfig;
import com.barber.web.app.model.AlipayTradePrecreteResResult;
import com.barber.web.app.service.AlipayMentOrderService;
import com.barber.web.entity.PayInfos;
import com.barber.web.entity.PeOrders;
import com.barber.web.utils.StringUtil;
...
import static com.barber.web.utils.enums.HttpCodeEnum.*;
...
public class AlipayMentOrderServiceImpl implements AlipayMentOrderService {
    ...
    public ApiResult getAliPayOrderStr(HttpServletRequest httpRequest, HttpServletResponse httpResponse, PeOrders orders) throws IOException {
        ...
        try{
            ...
            if(alipayTradeAppPayResponse.isSuccess()) {
                ...
                ApiResult apiResult = new ApiResult(CODE_0000.getCode(),CODE_0000.getMessage());
                apiResult.setData(resResult);
                return apiResult;
            }else{
                logger.info("加签异常！订单主键：["+orders.getId()+"]");
                return  new ApiResult(ALIPAY_CODE_6100.getCode(),ALIPAY_CODE_6100.getMessage());
            }
    ...
    private ApiResult checkOrderPrams(PeOrders orders){
        if(orders == null){
            return new ApiResult(ALIPAY_CODE_6170.getCode(),ALIPAY_CODE_6170.getMessage());
        }
        if(orders.getTotalPrice()==null){
            return new ApiResult(ALIPAY_CODE_6150.getCode(),ALIPAY_CODE_6150.getMessage());
        }
        ...
        if(StringUtil.isEmpty(orders.getProductName())){
            return new ApiResult(ALIPAY_CODE_6130.getCode(),ALIPAY_CODE_6130.getMessage());
        }
        if(orders.getId() == null){
            return new ApiResult(ALIPAY_CODE_6120.getCode(),ALIPAY_CODE_6120.getMessage());
        }
        if(orders.getCompaniesId() == null){
            return new ApiResult(ALIPAY_CODE_6160.getCode(),ALIPAY_CODE_6160.getMessage());
        }
        return null;
    }
...
//                                ApiResult apiResult = orderService.UpdatePayInfos(peOrders,tradeNo,conversionParams,dateFormat(gmtPayment),totalAmt, PayWayEnum.ALIPAY);
    ...
    public ApiResult checkAlipay(OrderCo orderCo) {
        ...
        if(payInfos == null || StringUtil.isEmpty(payInfos.getPreNum())){
            return new ApiResult(ALIPAY_CODE_6310.getCode(),ALIPAY_CODE_6310.getMessage());
        }else{
...
//                PeOrders record = peOrdersDao.selectByPrimaryKey(orderCo.getId());
                ...
                if(record == null){
                    return new ApiResult(ALIPAY_CODE_6310.getCode(),ALIPAY_CODE_6310.getMessage()+"订单Id："+orderCo.getId());
                }
                ...
                if(result == 0) {
                    ...
                    return new ApiResult(ALIPAY_CODE_6200.getCode(),ALIPAY_CODE_6200.getMessage());
                }else if(result == 3 || result == 2){
                    if(record.getIsPaid().equals(IsPaidEnum.YES)){
                        logger.info("=========支付成功，订单号:"+outTradeNo);
                        // 如果已支付 查支付表
                        return new ApiResult(CODE_0000.getCode(),CODE_0000.getMessage());
                        ...
                        //支付成功，订单支付状态异常
                        return new ApiResult(ALIPAY_CODE_6350.getCode(),"订单号："+outTradeNo+ALIPAY_CODE_6350.getMessage());
                    }
                }else{
                    logger.info("==================订单号："+outTradeNo+",该笔订单未能支付成功，支付状态："+ resultMessage);
                    return new ApiResult(ALIPAY_CODE_6500.getCode(),"订单号："+outTradeNo+ALIPAY_CODE_6500.getMessage());
                }
